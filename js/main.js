...
function buildProductRow(productRow){
    ...
    productRow.forEach(product => {
        ...
        buy.onclick = function() {localStorageSet(product.id, buildLocalStorageCartEntry(product)); chartCountHandle();}
        cardBody.append(buy);
...
function chartCountHandle(){
    let chartCountParam = document.getElementById("chart-count");
    chartCountParam.innerHTML = "";
    chartCountParam.innerHTML = getChartCount();
}
...
function getChartCount(){
    ...
    while ( i-- ) {
        ...
        chartCount += Number(item.split("  ")[2]);
    }
    if(chartCount == 0){
        return "";
    } 
